...
import LastFivePasswords from "./LastFivePasswords";
...
function App() {
  ...
  const getLocalItems = () => {
    let list = localStorage.getItem("passwords");
    if (list) {
  ...
  const [LastFivePassword, setLastFivePassword] = useState(getLocalItems());

  useEffect(() => {
    localStorage.setItem("passwords", JSON.stringify(LastFivePassword));
  }, [LastFivePassword]);
  ...
  const handleOnSubmit = (e) => {
    ...
    if (generatedPassword) {
      ...
      setLastFivePassword((prevState) => {
        return [...prevState, generatedPassword];
      });
    } else {
  ...
  return (
    <section>
      ...
      <div className="container">
        <LastFivePasswords passwords={LastFivePassword} />
      </div>
    </section>
  );
}
